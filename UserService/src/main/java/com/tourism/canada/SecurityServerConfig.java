...
public class SecurityServerConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/api/**/user/register", "/api/customer/authenticate", "/api/customer/verifytoken", "/api/customer/validate2FA/**").permitAll().and().authorizeRequests();

		
		http.cors().and().requestMatchers()
				.antMatchers( "/api/twoFactor/**", 
